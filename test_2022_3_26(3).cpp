...
#endif
//c++ 中 const 修饰 3
// const 修饰自定义数据类型，比如类对象，那么也会分配内存
// 
struct Person
{
	int num;
...
void test1()
{
	const Person person = { 100,"Lucy" };
	cout << "num = " << person.num << ", name = " << person.name << endl;
...
//总结：在 C++ 中
//1、const int date = 10;//date 先放入符号表
//2、如果对 date 取地址 系统才会个 date 开辟空间
//3、const int a = b; // b  是变量名 系统直接给 a 开辟空间 而不放人符号表
